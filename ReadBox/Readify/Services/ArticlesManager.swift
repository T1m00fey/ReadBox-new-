...
//        try await UserManager.shared.updateCreatedPosts(newPost: newId)
    ...
    func getCommsCount(at id: String) async throws -> Int {
        try await articleDocument(id: id).getDocument(as: CommentsCount.self).commentsCount ?? 0
    }
    ...
    func updateCommentsCount(at postId: String, isPlus: Bool) async throws {
        ...
        let commentsCount = try await articleDocument(id: postId).getDocument(as: CommentsCount.self).commentsCount ?? 0
        let nextCount = max(0, commentsCount - 1)
